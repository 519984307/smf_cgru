...
#     r'\[Redshift\] Exporting proxy \[frame (\d)\]'
# )

re_percent = re.compile(r'(Block*)(\s*)(\d*)(\/)(\d*)(\s*)(\S*)(\s*)(rendered by GPU.*)')
re_frame_start = re.compile(r'\[Redshift\] Exporting proxy \[frame (\d)\]')
re_frame_skip = re.compile(r'.*Skipping frame.*')
re_frame_done = re.compile(r'.*\[Redshift].*Saved proxy file.*')


class mayabatch_redshift(parser.parser):
